...
from dissertation_intro_qa.artifacts.provider import make_prompt_run_manifest, make_provider_result_artifact
...
class PromptRunner:
  ...
  def run(self, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prompt_text = render_prompt(template, {
    ...
    provider_artifact = make_provider_result_artifact(provider_result)
    write_json(run_dir / "provider_result.json", provider_artifact)

    audit_output_path = None
    if output_cfg.get("emit_audit_artifact", True):
      audit_artifact = map_provider_result_to_audit_artifact(
          provider_result,
          document_id=document_id,
          mapping_config=output_cfg.get("provider_mapping", {"mode": "demo_structured"}),
      )
      audit_output_path = run_dir / "mapped_audit.json"
      write_json(audit_output_path, audit_artifact)
    ...
    return {
        ...
        "provider_result_path": str(run_dir / "provider_result.json"),
        "mapped_audit_path": str(audit_output_path) if audit_output_path else "",
