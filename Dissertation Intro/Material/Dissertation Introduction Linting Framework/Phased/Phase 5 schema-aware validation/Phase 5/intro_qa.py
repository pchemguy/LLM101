...
- validate-audit: validate an audit JSON against schema-aware rules
- validate-comparison: validate a comparison JSON against schema-aware rules
- history-index: build a history index across multiple audit JSON reports
- dashboard: render a dashboard scaffold from a history index JSON
"""
...
def _validate_schema_node(payload: Any, schema: dict[str, Any], path: str = "$") -> list[str]:
    ...
    if schema_type and not _json_type_matches(str(schema_type), payload):
        errors.append(
            f"{path}: expected {schema_type}, got {type(payload).__name__}."
        )
        return errors
    ...
    if schema_type == "object":
        ...
        for key, child_schema in properties.items():
            if key in payload and isinstance(child_schema, dict):
                errors.extend(
                    _validate_schema_node(payload[key], child_schema, f"{path}.{key}")
                )
        ...
        if isinstance(item_schema, dict):
            for idx, item in enumerate(payload):
                errors.extend(
                    _validate_schema_node(item, item_schema, f"{path}[{idx}]")
                )
    return errors


def _validate_payload_against_schema(payload: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
    errors = _validate_schema_node(payload, schema)
    return {
        "passed": not errors,
...
def _history_index(reports: list[dict[str, Any]]) -> dict[str, Any]:
    trend = _trend_summary(reports)
    ...
    for idx, report in enumerate(reports, start=1):
        ...
        doc_id = _document_label(report, f"report_{idx}")
        doc_defect_codes: list[str] = []
        for defect in defects:
            code = str(defect.get("code", "")).strip()
            severity = str(defect.get("severity", "unknown")).lower()
            if code:
                defect_counter[code] += 1
                doc_defect_codes.append(code)
        ...
        entries.append(
            {
                ...
                "logical_integrity_score": float(summary.get("logical_integrity_score", 0.0)),
                "overall_verdict": summary.get("overall_verdict", ""),
                "scores": scores,
                "defect_codes": sorted(doc_defect_codes),
            }
        )
    ...
    return {
        ...
        "defect_frequencies": [
            {"code": code, "count": count} for code, count in defect_counter.most_common()
        ],
        "trend_summary": trend,
    }
...
def _render_dashboard_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# Dissertation Intro QA Dashboard ({payload.get('report_count', 0)} reports)",
        "",
        "## Severity Totals",
    ]
    sev = payload.get("severity_totals", {})
    if sev:
        for key in sorted(sev):
            lines.append(f"- {key}: {sev[key]}")
    else:
        lines.append("- нет")

    lines.extend(["", "## Defect Frequency Top 15"])
    freqs = payload.get("defect_frequencies", [])[:15]
    if freqs:
        for item in freqs:
    ...
    avg_scores = trend.get("average_scores", {})
    lines.extend(["", "## Average Scores"])
    if avg_scores:
        for key in sorted(avg_scores):
            lines.append(f"- {key}: {avg_scores[key]}")
    else:
        lines.append("- нет")
    ...
    if entries:
        for entry in entries:
            lines.extend([
                ...
                f"- audit_mode: {entry.get('audit_mode', '')}",
                f"- critical_count: {entry.get('critical_count', 0)}",
                f"- major_count: {entry.get('major_count', 0)}",
                f"- logical_integrity_score: {entry.get('logical_integrity_score', 0)}",
                f"- overall_verdict: {entry.get('overall_verdict', '')}",
                "",
            ])
    else:
        lines.append("- нет")
    ...
    lines.extend([
        "## Next Steps",
        "- Use the frequency table to prioritize recurring defect codes.",
        "- Re-audit after each targeted repair pass.",
        "- Track whether critical and major defects trend downward over versions.",
        "",
    ])
    return "\n".join(lines)
...
def cmd_validate_audit(args: argparse.Namespace) -> int:
    schema = _read_json(Path(args.schema)) if args.schema else DEFAULT_AUDIT_SCHEMA
    report = _read_json(Path(args.report))
    payload = _validate_payload_against_schema(report, schema)
    if args.out:
        _write_json(Path(args.out), payload)
...
def cmd_validate_comparison(args: argparse.Namespace) -> int:
    schema = _read_json(Path(args.schema)) if args.schema else DEFAULT_COMPARISON_SCHEMA
    report = _read_json(Path(args.report))
    payload = _validate_payload_against_schema(report, schema)
    if args.out:
        _write_json(Path(args.out), payload)
...
def cmd_history_index(args: argparse.Namespace) -> int:
    reports = _collect_audit_reports(args.reports)
    payload = _history_index(reports)
    _write_json(Path(args.out_json), payload)
    if args.out_md:
        _write_text(Path(args.out_md), _render_dashboard_markdown(payload))
    print(f"Wrote history index JSON: {args.out_json}")
    if args.out_md:
        print(f"Wrote history index Markdown: {args.out_md}")
    return 0


def cmd_dashboard(args: argparse.Namespace) -> int:
    payload = _read_json(Path(args.history_index))
    _write_text(Path(args.out), _render_dashboard_markdown(payload))
    print(f"Wrote dashboard Markdown: {args.out}")
    return 0
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_audit_parser = subparsers.add_parser(
        "validate-audit",
        help="Validate an audit JSON file against a schema-aware rule set.",
    )
    validate_audit_parser.add_argument("report", help="Path to audit JSON report.")
    validate_audit_parser.add_argument("--schema", help="Optional audit schema JSON path.")
    validate_audit_parser.add_argument("--out", help="Optional validation output JSON path.")
    validate_audit_parser.set_defaults(func=cmd_validate_audit)

    validate_comparison_parser = subparsers.add_parser(
        "validate-comparison",
        help="Validate a comparison JSON file against a schema-aware rule set.",
    )
    validate_comparison_parser.add_argument("report", help="Path to comparison JSON report.")
    validate_comparison_parser.add_argument("--schema", help="Optional comparison schema JSON path.")
    validate_comparison_parser.add_argument("--out", help="Optional validation output JSON path.")
    validate_comparison_parser.set_defaults(func=cmd_validate_comparison)

    history_parser = subparsers.add_parser(
        "history-index",
        help="Build a history index across multiple audit JSON files.",
    )
    history_parser.add_argument("reports", nargs="+", help="One or more audit JSON report paths.")
    history_parser.add_argument("--out-json", required=True, help="Output history index JSON path.")
    history_parser.add_argument("--out-md", help="Optional output Markdown dashboard path.")
    history_parser.set_defaults(func=cmd_history_index)

    dashboard_parser = subparsers.add_parser(
        "dashboard",
        help="Render a dashboard scaffold from a history index JSON file.",
    )
    dashboard_parser.add_argument("history_index", help="Path to history index JSON file.")
    dashboard_parser.add_argument("--out", required=True, help="Output Markdown path.")
