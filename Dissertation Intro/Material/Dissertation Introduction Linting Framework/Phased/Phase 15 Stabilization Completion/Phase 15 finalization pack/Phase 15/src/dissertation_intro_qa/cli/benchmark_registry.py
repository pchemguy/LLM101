...
from dissertation_intro_qa.artifacts.registry import make_project_dashboard_artifact, make_run_registry_artifact, make_session_artifact
...
def main(argv: List[str] | None = None) -> int:
  try:
    ...
    if args.cmd == "create-session":
      ...
      assert_valid_artifact(artifact, expected_type="session", allow_legacy=False)
    ...
    if args.cmd == "build-registry":
      ...
      assert_valid_artifact(artifact, expected_type="run_registry", allow_legacy=False)
      write_json(Path(args.out_json), artifact)
      if args.out_md:
    ...
    if args.cmd == "project-dashboard":
      ...
      assert_valid_artifact(artifact, expected_type="project_dashboard", allow_legacy=False)
      write_json(Path(args.out_json), artifact)
      if args.out_md:
