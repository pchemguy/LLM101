...
The repository is organized around a small set of conceptually distinct files. Some define the framework itself, some explain the conceptual basis of the framework, and some serve as substantive inputs for individual evaluations.
...
The standard is not an execution script. It does not define a scoring scale, the exact evaluation workflow, or the final report template. Instead, it provides the conceptual and normative basis that the operational protocol relies on.
...
#### [`Protocol for Structural, Semantic, and Specialty Alignment Evaluation of Dissertation Introductions`](./Protocol%20for%20Structural%2C%20Semantic%2C%20and%20Specialty%20Alignment%20Evaluation%20of%20Dissertation%20Introductions.md)
...
The protocol is designed especially for LLM-based evaluation, but it is also suitable for disciplined manual review. It is the document to use when you want to run the framework on a real dissertation introduction.

Typical uses include prompt engineering, structured dissertation review, testing evaluation workflows, and future automation of the analysis process.

### Conceptual background file

#### [`Russian Dissertation Introduction as Research Logic, Administrative Form, and Project Specification`](./Russian%20Dissertation%20Introduction%20as%20Research%20Logic%2C%20Administrative%20Form%2C%20and%20Project%20Specification.md)
...
To use the framework for a concrete evaluation, first identify the three substantive documents needed for analysis: the evaluation standard, the protocol, and the case materials. In practice, that means you should have the standard file, the protocol file, the relevant VAK specialty passport, and the target dissertation introduction. The conceptual background file is optional, but useful if you want to understand the theoretical basis of the framework.
...
A minimal working sequence is therefore: review the standard, open the protocol, supply the passport and the dissertation introduction, and run the analysis according to the protocol’s stages. The result should be a structured diagnostic report in Russian, with explicit evidence, criterion-level judgments, severity-classified defects, and revision priorities.
